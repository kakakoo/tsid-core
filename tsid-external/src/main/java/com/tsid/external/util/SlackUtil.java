...
import com.tsid.external.api.slack.dto.StatisticDto;
...
public class SlackUtil {
    ...
    public static JSONObject makeAdminError(String serverName, String url, String exception, String date, String message) {
    ...
    public static JSONObject makeStatisMessage(String statisticName, StatisticDto statisticDto) {

        String data = "가입인원 : " + statisticDto.getJoinUser() + "\n" +
                "인증횟수 : " + statisticDto.getCertRequest() + "\n" +
                "인증완료 : " + statisticDto.getCertDone();

        String excute = "주간 업데이트 : " + statisticDto.getWeeklyExecute() + "\n" +
                "월간 업데이트 : " + statisticDto.getMonthlyExecute();

        List<JSONObject> blocks = new ArrayList<>();
        JSONObject header = makeHeader(statisticName);
        JSONObject dateSection = makeSection("배치 날짜", statisticDto.getDate());
        JSONObject statisticData = makeSection("data", data);
        JSONObject executeData = makeSection("execute", excute);

        blocks.add(header);
        blocks.add(dateSection);
        blocks.add(statisticData);
        blocks.add(executeData);
        JSONObject result = new JSONObject();
        result.put("blocks", blocks);
        return result;
    }
